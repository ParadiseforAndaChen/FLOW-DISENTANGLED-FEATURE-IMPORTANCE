    ...
    def _single_fold_importance(
        ...
        **kwargs
        ...
        if self.show_tqdm and (j is None) and d > 1:
            iterator = tqdm(feats, desc="CPI (X-space) conditional permutation")
...
class CPIZ_Flow_Model_Estimator(ImportanceEstimator):
    ...
    name: str = field(default="CPI_Z_Flow_Model_New", init=False)
    ...
    def _encode_to_Z(self, X: np.ndarray) -> np.ndarray:
        assert self.flow_model is not None, "flow_model not set"
        import torch
        with torch.no_grad():
    ...
    def _single_fold_importance(
        ...
        **kwargs
        ...
        assert hasattr(self, "mu_hat") and (self.mu_hat is not None), "请先调用 fit() 训练 mu_hat。"
        n, d = X.shape
        ...
        for jj in iterator:
            ...
            if self.sampling_method == "resample":
                ...
                assert self.Z_full is not None, "Z_full is None."
...
class CPI_Flow_Model_Estimator(ImportanceEstimator):
    ...
    # {"resample", "permutation", "normal", "condperm"}
    ...
    H_max_samples: int = 0         
    H_: np.ndarray | None = field(default=None, init=False)  # (d, d)
...
class SCPIZ_Flow_Model_Estimator(ImportanceEstimator):
    ...
    permuter: any = field(default_factory=lambda: RandomForestRegressor(
    n_estimators=500,
    max_depth=None,
    min_samples_leaf=5,
    random_state=0,  
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, j: Optional[int] = None):
        assert self.flow_model is not None, "flow_model is None."
    ...
    def _single_fold_importance(
        ...
        **kwargs
        ...
        for j_idx in iterator:
            ...
            ueifs[:, j_idx] = (y - y_perm) ** 2 - base_loss  
    ...
    def _compute_single_feature_importance(
        ...
        if not self.exact:
            def sample_coalitions_with_weights(coalitions: List[Tuple[Tuple[int, ...], Tuple[int, ...]]], weights: np.ndarray, n_samples: int, rng: np.random.Generator) -> List[Tuple[Tuple[int, ...], Tuple[int, ...]]]:
                """Sample coalitions directly using probability weights."""
...
class DFIZEstimator(ImportanceEstimator):
    """
    Variant of DFI that returns phi_Z (importance in disentangled space).
    """
