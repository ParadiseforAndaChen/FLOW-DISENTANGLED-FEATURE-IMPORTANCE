...
class ImportanceEstimator(abc.ABC):
    ...
    random_state: int = 0
    regressor: any = field(default_factory=lambda: RandomForestClassifier(
        n_estimators=500,
        max_depth=None,
        min_samples_leaf=5,
        random_state=0,
...
class CPI_Flow_Model_Estimator_cls(ImportanceEstimator):
    ...
    def _single_fold_importance(
        ...
        **kwargs
        ...
        np.save("ctg_ueifs.npy", ueifs)              # latent 空间 EIF: ω_ij, shape (n, d)
        np.save("ctg_H_T.npy", self.H_.T)            # H_T[j, l], shape (d, d)
        np.save("ctg_ueifs_mapped.npy", ueifs_mapped)  # case-wise FDFI(X), shape (n, d)   
